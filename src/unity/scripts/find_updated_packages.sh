...
for orig_rpm in $orig_rpms; do
	...
	FOUND_OK=0
	orig_rpm_name=`rpmquery --queryformat="%{NAME}" -p $orig_rpm`
	orig_rpm_arch=`rpmquery --queryformat="%{ARCH}" -p $orig_rpm`
	respin_rpms=`find /mnt/respin-mnt/respin/ -type f -name $orig_rpm_name*.rpm`

	for respin_rpm in $respin_rpms; do
		respin_rpm_name=`rpmquery --queryformat="%{NAME}" -p $respin_rpm`
		respin_rpm_arch=`rpmquery --queryformat="%{ARCH}" -p $respin_rpm`
